#!/usr/bin/env python3
"""Audit Capital IQ total-liabilities candidate exports.

The current Capital IQ screener route exposes ordinary "Total Liabilities" as
SNL_TOTAL_LIAB. This audit treats it as a usable raw candidate only when target
FY parameters and numeric coverage are present, while preserving the source
family warning for later manuscript/data decisions.
"""
...
TOTAL_LIAB_KEYS = {"SNL_TOTAL_LIAB", "SP_TOTAL_LIAB", "SP_TOTAL_LIABILITIES"}
PREFERRED_TOTAL_LIAB_KEYS = {"SP_TOTAL_LIAB", "SP_TOTAL_LIABILITIES"}
LEAKAGE_PARAMS = {
    "Latest Fiscal Year",
...
def field_coverage(
    ...
    return records, {
        ...
        "has_preferred_key": any(key in PREFERRED_TOTAL_LIAB_KEYS for key in available_keys),
        "has_candidate_key": any(key in TOTAL_LIAB_KEYS for key in available_keys),
    }
...
def render_report(
    ...
    lines = [
        ...
        "",
        f"- Total-liabilities field keys detected: {meta['available_keys']}",
        f"- Preferred CIQ total-liabilities key detected: {meta['has_preferred_key']}",
    ...
    if not meta["has_candidate_key"]:
        hard_failures.append("No recognized total-liabilities candidate key detected.")
    if meta["missing_years"]:
        hard_failures.append(f"Missing target total-liabilities FY years: {meta['missing_years']}.")
    if not meta["has_preferred_key"]:
        warnings.append(
            "`SNL_TOTAL_LIAB` is a usable raw candidate but not yet a preferred CIQ standard key; "
            "retain this warning until methodology reconciles the source-family choice."
        )
    ...
    if hard_failures:
        ...
        lines.append("- Target-year total-liabilities columns are present and have auditable numeric coverage.")
